...
class DocumentationGeneratorAgent < Agents::Agent
  ...
  instructions <<~PROMPT
    ...
    2. Create a file using the filesystem tools in the HOME directory
    3. Use an appropriate filename like "api_authentication_guide.md" or "rate_limiting_guide.md"
    4. Use the write_file tool with the full path: /HOME_directory/filename.md
    5. Add the filename to context.saved_files array
    6. Confirm what file was created and where it was saved
    
    IMPORTANT: Always use the full path starting with /HOME_directory/ when saving files.
  PROMPT
